...
export default function EditListingPage() {
  ...
  const onSubmit = async (formData: ListingFormData) => {
    const { error } = await supabase
      .from('listings')
      .update({ ...formData, status: 'pending' }) // Set status to pending for re-review
      .eq('id', params.id);

    if (error) {
      toast({ title: 'Error', description: 'Failed to update listing.', variant: 'destructive' });
    } else {
      toast({ title: 'Success', description: 'Listing updated and submitted for review.' });
      router.push('/dashboard/listings');
    }
  };
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <Select onValueChange={(value) => reset({ ...listing, condition: value })} defaultValue={listing.condition}>
                <SelectTrigger>
